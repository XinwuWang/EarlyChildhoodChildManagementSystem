...
const CMessageDetail = () => {
    const childId = localStorage.getItem('childId')
    const navigate = useNavigate()
    const [messageDetail, setMessageDetail] = useState([])
    const { id } = useParams()
    useEffect(() => {
        axios.get('http://localhost:3000/child/message_detail/' + id)
            .then(result => {
    ...
    const handleDelete = (id) => {
        if (window.confirm("ALERT! Are you sure you want to delete this record?")) {
            axios.delete(`http://localhost:3000/child/delete_message/${id}`)
                .then(result => {
                                ...
                                <div className="d-flex justify-content-center">
                                    <div className="dropdown">
                                        ...
                                        <ul className="dropdown-menu" aria-labelledby="replyDropdown">
                                            <li>
                                                <Link className="dropdown-item" to="/child_dashboard/message_admin">
                                                    Admin
                                                </Link>
                                            </li>
                                            <li>
                                                <Link className="dropdown-item" to="/child_dashboard/message_a_teacher">
                                                    Teacher
                                                </Link>
                                            </li>
                                        </ul>
                                    ...
                                    <Link to={`/child_dashboard/message/${childId}`} className="btn btn-secondary me-2" >Return</Link>
                                </div>
