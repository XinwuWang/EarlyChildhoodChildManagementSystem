...
const AMessageDetail = () => {
    ...
    const { id } = useParams()
    useEffect(() => {
        axios.get('http://localhost:3000/auth/message_detail/' + id)
            .then(result => {
    ...
    const handleDelete = (id) => {
        if (window.confirm("ALERT! Are you sure you want to delete this record?")) {
            axios.delete(`http://localhost:3000/auth/delete_message/${id}`)
                .then(result => {
                    if (result.data.Status) {
                        navigate(`/dashboard/message/${adminId}`);
                    } else {
                                ...
                                <div className="d-flex justify-content-center">
                                    <div className="dropdown">
                                        ...
                                        <ul className="dropdown-menu" aria-labelledby="replyDropdown">
                                            <li>
                                                <Link className="dropdown-item" to="/dashboard/message_a_teacher">
                                                    Teacher
                                                </Link>
                                            </li>
                                            <li>
                                                <Link className="dropdown-item" to="/dashboard/message_a_child">
                                                    Child
                                                </Link>
                                            </li>
                                        </ul>
                                    ...
                                    <Link to={`/dashboard/message/${adminId}`} className="btn btn-secondary me-2" >Return</Link>
                                </div>
