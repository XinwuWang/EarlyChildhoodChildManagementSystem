...
const TSignOut = () => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        const { attendance_date, child_id, time_out, teacher_signature } = attendance;

        if (!child_id || !time_out) {
            return alert('Please select a child and enter the time out');
        }

        const data = { attendance_date, child_id, time_out, teacher_signature };

        axios.post('http://localhost:3000/teacher/check_sign_in', { child_id, attendance_date })
            .then(result => {
                if (result.data.Status) {
                    axios.post('http://localhost:3000/teacher/check_sign_out', { child_id, attendance_date })
                        .then(result => {
                            if (result.data.Status) {
                                window.confirm('This child has already signed out today.');
                            } else {
                                axios.post('http://localhost:3000/teacher/sign_out', data)
                                    .then(result => {
                                        if (result.data.Status) {
